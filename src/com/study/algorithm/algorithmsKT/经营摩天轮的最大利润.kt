...
class 经营摩天轮的最大利润 {
    ...
    fun minOperationsMaxProfit(customers: IntArray, boardingCost: Int, runningCost: Int): Int {
        //模拟
        //模拟摩天轮运营
        //记录每次转动时的总利润profit,记录当前最大利润以及对应的转动次数max[],记录每次转动时剩余排队人数wait
        //当wait>4时,profit += 4 * boardingCost - runningCost,否则profit += wait * boardingCost - runningCost
        //如果当前利润比之前记录的最大利润大,则更新最大的转动次数以及对应的利润

        //如果每次坐满4个人的票价还要小于等于一次转动的费用,那么就不需要运营了
        if (4 * boardingCost <= runningCost) {
            return -1
        }

        var wait = 0
        val max = intArrayOf(-1, 0)
        var count = 0
        var profit = 0
